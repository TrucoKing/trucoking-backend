...
app.get('/roleta/estado', authMiddleware, async (req, res) => {
  try {
    ...
    const e = r.rows[0];
    res.json({
      custo_giro: parseFloat(e.custo_giro),
...
app.post('/admin/roleta/config', async (req, res) => {
  if (!adminOk(req)) return res.status(401).json({ erro: 'Nao autorizado' });
  const { custo_giro, gatilho, premio_pequeno_max } = req.body;
  await db.query(
    `UPDATE roleta_estado SET
       custo_giro = COALESCE($1, custo_giro),
       gatilho = COALESCE($2, gatilho),
       premio_pequeno_max = COALESCE($3, premio_pequeno_max),
       atualizado_em=NOW() WHERE id=1`,
    [custo_giro, gatilho, premio_pequeno_max]
  );
  const r = await db.query('SELECT * FROM roleta_estado WHERE id=1');
