...
if (!nome || !cpf || !email || !senha) return res.status(400).json({ erro: ‘Campos obrigatorios faltando’ });
...
} catch (err) { console.error(err); res.status(500).json({ erro: ‘Erro ao fazer login’ }); }
});
...
} catch (err) { console.error(err); res.status(500).json({ erro: ‘Erro ao gerar Pix’ }); }
});

app.get(’/deposito/status/:id’, auth, async (req, res) => {
const r = await db.query(‘SELECT * FROM transacoes WHERE id=$1 AND user_id=$2’, [req.params.id, req.user.id]);
if (!r.rows[0]) return res.status(404).json({ erro: ‘Nao encontrado’ });
res.json({ status: r.rows[0].status, valor: r.rows[0].valor });
});
...
console.log(‘Deposito aprovado: R$’ + valor + ’ user#’ + userId);
} catch (err) { console.error(err); }
});

app.post(’/saque/solicitar’, auth, async (req, res) => {
const { valor, pix_chave } = req.body;
if (!valor || valor < 20) return res.status(400).json({ erro: ‘Minimo R$ 20,00’ });
...
await db.query(‘INSERT INTO transacoes (user_id,tipo,valor,status,pix_chave) VALUES ($1,$2,$3,$4,$5)’, [req.user.id, ‘saque’, valor, ‘processando’, pix_chave]);
res.json({ ok: true, mensagem: ‘Saque solicitado! Pix em ate 5 minutos.’ });
} catch (err) { console.error(err); res.status(500).json({ erro: ‘Erro ao solicitar saque’ }); }
});
...
if (req.headers[‘x-admin-key’] !== process.env.ADMIN_KEY) return res.status(403).json({ erro: ‘Acesso negado’ });
const r = await db.query(‘SELECT t.*,u.nome,u.email FROM transacoes t JOIN users u ON t.user_id=u.id ORDER BY t.criado_em DESC LIMIT 200’);
res.json(r.rows);
});

setupDatabase().then(() => {
app.listen(port, () => console.log(’TrucoKing Server porta ’ + port));
