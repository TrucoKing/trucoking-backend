...
app.post('/saque/solicitar', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Debita saldo e cria transação pendente
    ...
    // Envia Pix via Mercado Pago
    try {
      const transferencia = await mpPayment.create({
        body: {
          transaction_amount: Number(valor),
          description: 'TrucoKing - Saque',
          payment_method_id: 'pix',
          payer: { email: req.user.email },
          receiver: {
            identification: { type: pix_tipo?.toUpperCase() || 'EMAIL', number: pix_chave }
          },
          metadata: { user_id: req.user.id, tipo: 'saque', tx_id: tx.rows[0].id }
        }
      });

      await db.query(
        'UPDATE transacoes SET status=$1, mp_payment_id=$2 WHERE id=$3',
        ['processando', String(transferencia.id), tx.rows[0].id]
      );

      res.json({
        ok: true,
        mensagem: 'Saque solicitado! Pix enviado em até 5 minutos.',
        transacao_id: tx.rows[0].id
      });
    } catch (mpErr) {
      // Se falhar o envio, estorna o saldo
      await db.query('UPDATE users SET saldo = saldo + $1 WHERE id=$2', [valor, req.user.id]);
      await db.query('UPDATE transacoes SET status=$1 WHERE id=$2', ['erro', tx.rows[0].id]);
      console.error('Erro ao enviar Pix:', mpErr);
      res.status(500).json({ erro: 'Erro ao processar saque. Saldo estornado.' });
    }
  } catch (err) {
    console.error(err);
